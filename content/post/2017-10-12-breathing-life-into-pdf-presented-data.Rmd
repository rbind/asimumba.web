...
fig_width: 2
fig_height: 2
bigimg: [{src: "/img/file.jpg", desc: "Files"}]
output:
   blogdown::html_page:
     toc: true
---
```{r,echo=FALSE}
knitr::opts_chunk$set(echo = TRUE,tidy = TRUE, eval = TRUE)
```
...
cso_table <- extract_tables(cso, pages = 24, 
                            method = "data.frame") # We are going to pass the cso variable to the extract_tables() function
cso_column <- cso_table[[1]] # The table of interest is on page 24, the other arguments are left as defaults
```
...
write.csv(cso_column, file = "../../static/data/cso_data.csv",
          row.names = FALSE) # I have passed a relative path where I want the CSV file to be stored
```
...
cso_data <- cso_column[-c(1,2,33:36), ] # this eleminates the rows specified
...
```{r,message=FALSE}
library(tidyverse) # install.packages("tidyverse") This will install the tidyverse
cso_data <- cso_data %>% 
  as.tibble() # converting to a tibble
