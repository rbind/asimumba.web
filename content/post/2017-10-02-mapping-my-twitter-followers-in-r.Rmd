...
Since the data has to be mined from twitter to R. The first issue to address is setting up this link to twitter. Luckily, the twitter's Application Programmming Interface (API) handles this integration. Twitter has a developer site, where you can set-up your app and acquire the authentication keys and tokens  needed for R and twitter to communicate. For set-up instructions the following [post](https://www.credera.com/blog/business-intelligence/twitter-analytics-using-r-part-1-extract-tweets/) has a nice  step by step walk through set-up from the twitter side.
...
I call the `twitterMap.R` script through `source()`, the script loads the function in the work environment, in order to user the `twitterMap()` command. 
...
For both the followers and those I follow, it can be seen that they are mainly located in North America, Europe and Africa. It is my hope to follow more of the budding tech and finance enthusiats on the African continent. Next time I mine this data, it should paint a slightly more balanced picture.